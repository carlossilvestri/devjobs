...
exports.verificarUsuario = (req, res, next) => {
    ...
    if (req.isAuthenticated()) {
        return next(); //Para que se salga de este middleware. Estan autenticados.
    }
    //Redireccionar
    res.redirect('/iniciar-sesion'); //No estan autenticados
}

exports.mostrarPanel = async(req, res) => {
    //Consultar el usuario autenticado
    const vacantes = await Vacante.find({ autor: req.user._id });
    // console.log('En mostrarPanel');
...
exports.reestablecerPassword = async(req, res, next) => {
        const usuario = await Usuarios.findOne({
            ...
            expira: { $gt: Date.now() } //$gt greater than
        });
        if (!usuario) {
...
exports.guardarPassword = async(req, res) => {
    ...
    await usuario.save();
    //Redirigir
    req.flash('correcto', 'Contraseña modificado correctamente.');
    res.redirect('/iniciar-sesion');
